...
public partial class CurveComparison : Node
{
    ...
    public double ReferenceCurve(double x)
    {
        ...
        return allenwp_piecewise_reinhard(x, .18, .18, white, 1.0, A, lowClip, true);
    ...
    public double allenwp_curve_cpu_code(double x, double awp_contrast, double awp_high_clip, double output_max_value)
    {
        ...
        double awp_toe_a = ((1.0 / awp_crossover_point) - 1.0) * Math.Pow(awp_crossover_point, awp_contrast);
        ...
        double awp_shoulder_max = output_max_value - awp_crossover_point;
    ...
    public double allenwp_piecewise_reinhard(double x, double midIn = 0.18, double midOut = 0.18, double white = 16.2917402385381, double maxVal = 1.0, double contrast = 1.25652780401491, double lowClip = 1.0, bool scaleWhite = true)
    {
        // CPU side calculations:
        //maxVal = Math.Max(maxVal, 1.0);
        if (scaleWhite)
